...
- ✨ 新增混合检索工具（hybrid_search_tool.py）
- ✨ 新增混合检索机制（知识库 70% + 联网搜索 30%）
- ✨ 新增结果融合算法（加权排序 + 去重）
- ✨ 新增 Agent 逻辑流程与优化方案文档

### 优化

- 🚀 强制所有技术问题使用混合检索工具
- 🚀 优化工具调用顺序（混合检索 > 知识库 > 联网搜索）
- 🚀 优化 System Prompt，明确工具调用策略
- 🚀 优化回答规范（结构 + 长度 + 来源标注）
- 🚀 在 agent.py 中注册混合检索工具（最高优先级）
...
- 📚 新增 `src/tools/hybrid_search_tool.py` - 混合检索工具
- 📚 新增 `docs/agent_logic_flow_and_optimization.md` - Agent 逻辑流程与优化方案
- 📚 更新 `config/agent_llm_config.json` - 优化 System Prompt
- 📚 更新 `src/agents/agent.py` - 注册混合检索工具
...
- ✨ 导入 GBase8a 官方文档到知识库（下载中心、产品手册）
- ✨ 新增知识库检索工具优化，默认搜索 gbase8a 数据集
- ✨ 新增回答策略优化（优先级、长度控制、深度要求）
- ✨ 新增多轮对话引导机制（最多 3 轮）
- ✨ 新增 Agent 优化方案文档

### 优化

- 🚀 System Prompt 大幅精简，提高可读性
- 🚀 明确回答优先级（知识库 > 联网搜索 > LLM 自身能力）
- 🚀 限制回答长度（50-500 字）
- 🚀 降低 max_completion_tokens 从 8000 到 4000
- 🚀 要求回答包含原理、建议、注意事项
...
- 📚 新增 `docs/agent_optimization_plan.md` - Agent 回答质量优化方案
- 📚 更新 `config/agent_llm_config.json` - 优化 System Prompt
